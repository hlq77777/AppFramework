...
public class WaveLoadingView extends View {
    ...
    private static final int DEFAULT_BORDER_COLOR = Color.parseColor("#212121");

    private static final int OUT_BORDER_WIDTH = 0;
    private static final int OUT_BORDER_COLOR = Color.parseColor("#212121");
    ...
    private static final float DEFAULT_OUT_BORDER_MARGIN = 3;
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        mOutBorderMargin= DimensionUtil.getSizeInPixels(DEFAULT_OUT_BORDER_MARGIN,context);
        // Init Wave.
        ...
        mBorderPaint.setStrokeWidth(attributes.getInteger(R.styleable.WaveLoadingView_waveBorderWidth, DEFAULT_BORDER_WIDTH));
        mBorderPaint.setColor(attributes.getColor(R.styleable.WaveLoadingView_waveBorderColor, DEFAULT_BORDER_COLOR));

        mOutBorderPaint = new Paint();
        mOutBorderPaint.setAntiAlias(true);
        mOutBorderPaint.setStyle(Paint.Style.STROKE);
        mOutBorderPaint.setStrokeWidth(attributes.getInteger(R.styleable.WaveLoadingView_outBorderWidth, OUT_BORDER_WIDTH));
        mOutBorderPaint.setColor(attributes.getColor(R.styleable.WaveLoadingView_outBorderColor, OUT_BORDER_COLOR));
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (mWaveShader != null) {
            ...
            mShaderMatrix.postTranslate(mWaveShiftRatio * getWidth(),
                    (DEFAULT_WATER_LEVEL_RATIO - mWaterLevelRatio) * getHeight()-mOutBorderMargin);
                ...
                case 0:
                    if (borderWidth > 0) {
                        canvas.drawCircle(getWidth() / 2f, getHeight() / 2f,
                                (getWidth() - borderWidth) / 2f - 1f-mOutBorderMargin, mBorderPaint);
                    }
                    if (outBorderWidth > 0) {
                        canvas.drawCircle(getWidth() / 2f, getHeight() / 2f,
                                (getWidth() - outBorderWidth) / 2f - 1f, mOutBorderPaint);
                    }
                    float radius = getWidth() / 2f - borderWidth;
                    canvas.drawCircle(getWidth() / 2f, getHeight() / 2f, radius-mOutBorderMargin, mWavePaint);

                    break;
                case 1:
                    if (borderWidth > 0) {
                        canvas.drawRect(
                                borderWidth / 2f+mOutBorderMargin/2,
                                borderWidth / 2f+mOutBorderMargin/2,
                                getWidth() - borderWidth / 2f - 0.5f-mOutBorderMargin/2,
                                getHeight() - borderWidth / 2f - 0.5f-mOutBorderMargin/2,
                                mBorderPaint);
                    }
                    if (outBorderWidth > 0) {
                        canvas.drawRect(
                                outBorderWidth / 2f,
                                outBorderWidth / 2f,
                                getWidth() - outBorderWidth / 2f - 0.5f,
                                getHeight() - outBorderWidth / 2f - 0.5f,
                                mOutBorderPaint);
                    }
                    canvas.drawRect(borderWidth+mOutBorderMargin/2, borderWidth+mOutBorderMargin/2, getWidth() - borderWidth-mOutBorderMargin/2,
                            getHeight() - borderWidth-mOutBorderMargin/2, mWavePaint);
                    break;
            }
...
//        mWaveShader = null;
    ...
    public void setProgressValue(double progress) {
        ObjectAnimator waterLevelAnim = ObjectAnimator.ofFloat(this, "waterLevelRatio",0 ,((float) progress / 100) );
        ...
        setCenterTitle(String.format("%.2f%%",progress));
    }
